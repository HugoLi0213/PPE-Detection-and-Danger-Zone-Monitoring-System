...
def manage_zones(mode, input_source):
    if input_type == "image":
        image = cv2.imread(input_source)
        if image is None:
            raise ValueError("Failed to load image")
        zone_manager(image, mode)
    else:
        cap = cv2.VideoCapture(input_source if input_type == "video" else camera_index)
        if not cap.isOpened():
...
def zone_manager(image, mode):
    img_oh = image.shape[0]
    img_ow = image.shape[1]
    ...
    if mode == "draw":
        ...
        def draw(event, x, y, flags, param):
            ...
            if event == cv2.EVENT_MBUTTONDOWN:
                if dot_array:
                    temp_arr = [[int(dot[0] * x_ratio), int(dot[1] * y_ratio)] for dot in dot_array]
        ...
        if os.path.exists(zone_text_path):
            ...
            if zone_list:
                for zone in zone_list:
                    zone_scaled = [[int(pt[0] / x_ratio), int(pt[1] / y_ratio)] for pt in zone]
                    cv2.polylines(img, [np.array(zone_scaled)], True, (255, 0, 0), 2)
        ...
        if os.path.exists(zone_text_path):
            ...
            def remove(event, x, y, flags, param):
                ...
                img_copy = img.copy()
                if event == cv2.EVENT_LBUTTONDOWN:
                    arr_index = (arr_index + 1) % len(zone_list)
                elif event == cv2.EVENT_RBUTTONDOWN:
                    remove_arr[arr_index] = not remove_arr[arr_index]
                elif event == cv2.EVENT_MBUTTONDOWN:
                    new_zones = [zone for i, zone in enumerate(zone_list) if not remove_arr[i]]
                    ...
                    cv2.destroyAllWindows()
                    return
                zone_scaled = [[int(pt[0] / x_ratio), int(pt[1] / y_ratio)] for pt in zone_list[arr_index]]
                color = red if remove_arr[arr_index] else green
                cv2.polylines(img_copy, [np.array(zone_scaled)], True, color, 2)
                cv2.imshow("RemoveZone", img_copy)
            img_copy = img.copy()
            zone_scaled = [[int(pt[0] / x_ratio), int(pt[1] / y_ratio)] for pt in zone_list[arr_index]]
            cv2.polylines(img_copy, [np.array(zone_scaled)], True, green, 2)
            cv2.imshow("RemoveZone", img_copy)
...
@app.route('/manage_zone/<mode>')
def manage_zone(mode):
    if mode not in ["draw", "view", "delete"]:
        return jsonify({"error": "Invalid mode"}), 400
    try:
        manage_zones(mode, args.Input if input_type in ["video", "image"] else camera_index)
        load_zones()  # Reload zones after management
...
if __name__ == '__main__':
    socketio.run(app, host='127.0.0.1', port=5000, debug=True)#test:py main.py --Input "C:\\Users\\user\\Downloads\\PPE-Detection-and-Danger-Zone-Monitoring-System (1)\\PPE-Detection-and-Danger-Zone-Monitoring-System\\video\\BM02.mp4"